...
{
    public class JobGetNewMetricsRam : IJob
    {
        public async Task Execute(IJobExecutionContext context)
        {
            ...
            string address = Actions.GetConnectionAddress("ram", myUrl, fromTime);
        ...
        {
            RamMetricsResponse ramMetrics = new RamMetricsResponse();
                ...
                using (HttpClient httpClient = new HttpClient())
                {
                    using (HttpResponseMessage response = await httpClient.GetAsync(requestAddress))
                    {
                        using (HttpContent content = response.Content)
                        {
                            ...
                            ramMetrics = JsonConvert.DeserializeObject<RamMetricsResponse>(data);
