...
{
    public class JobGetNewMetricsDotNet : IJob
    {
        public async Task Execute(IJobExecutionContext context)
        {
            ...
            string address = Actions.GetConnectionAddress("dotnet", myUrl, fromTime);
        ...
        {
            DotNetMetricsResponse DotNetMetrics = new DotNetMetricsResponse();
                ...
                using (HttpClient httpClient = new HttpClient())
                {
                    using (HttpResponseMessage response = await httpClient.GetAsync(requestAddress))
                    {
                        using (HttpContent content = response.Content)
                        {
                            ...
                            DotNetMetrics = JsonConvert.DeserializeObject<DotNetMetricsResponse>(data);
